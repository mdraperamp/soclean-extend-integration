...
define(['N/format',
    ...
    function (format, search)
    {
         ...
         */
        lib.stringToDate = function (str)
        {
            return !str ? '' : format.parse({
                value: str,
                type: 'date'
            });
        };
         ...
         * setting
         */
        lib.dateToString = function (dateObj)
        {
         ...
         */
        lib.roundToFloat = function (flNumber, intDecimalPlaces)
        {
            //Default Decimal Places to 2
            if(lib.isEmpty(intDecimalPlaces))
            {
                intDecimalPlaces = 2;
            }
         ...
         */
        lib.formatCurrency = function (flValue, stCurrencySymbol, intDecimalPrecision)
        {
         ...
         */
        lib.search = function (stRecordType, stSearchId, arrSearchFilter, arrSearchColumn, bUseFilterExpressions, arrSearchSetting)
        {
        ...
        */
        lib.getCurrencyInfo = function ()
        {
            ...
            var arrCurrencySearchResults = lib.search('currency', null, arrFilters, arrColumns);
        ...
        /*
        *	Gets Subsidiary Currencys
        */
        lib.getSubsidiaryCurrency = function ()
        {
            var objSubsidiaryCurrency = {};

            var arrFilters = [];
            var arrColumns = [];
            arrColumns.push(search.createColumn({name: 'internalid', sort: 'ASC'}));
            arrColumns.push(search.createColumn({name: 'currency'}));

            var arrSubsidiarySearchResults = lib.search('subsidiary', null, arrFilters, arrColumns);
            if(arrSubsidiarySearchResults.length == 0)
            {
                throw 'Subsidiaries have not yet been configured. Contact your administrator.';
            }
            else
            {
                for(var i = 0; i < arrSubsidiarySearchResults.length; i++)
                {
                    var stSubsidiaryId = arrSubsidiarySearchResults[i].getValue({name: 'internalid'});
                    var stCurrencyId = arrSubsidiarySearchResults[i].getValue({name: 'currency'});
                    objSubsidiaryCurrency[stSubsidiaryId] = stCurrencyId;
                }
            }
            return objSubsidiaryCurrency;
        };

        /**
         * Convert item record type to its corresponding internal id (e.g. 'invtpart' to 'inventoryitem')
         */
        lib.toItemInternalId = function (stRecordType)
        {
        ...
        /**
         * Determine whether date is within period
         */
        lib.isDateInPeriod = function (stPeriodId, stDate)
        {
            var blIsDateInPeriod = false;

            var objPdSearch = search.create({
                type: 'accountingperiod',
                filters:
                    [
                        ['internalid', 'is', stPeriodId], 'AND',
            ...
            {
                blIsDateInPeriod = true;
                return blIsDateInPeriod;
            });
         ...
         */
        lib.getMonthsBetweenTwoDays = function(objStartDate, objEndDate)
        {
            return (objEndDate.getFullYear() - objStartDate.getFullYear()) * 12 +
                (objEndDate.getMonth() - objStartDate.getMonth());
        };

        /*
        *	Returns the period the date is in
        */
        lib.getPeriod = function (stDate)
        {
            ...
            var stPeriodId = '';
            var arrFilters = [];
            arrFilters.push(search.createFilter({name: 'closed', operator: 'is', values: ['F']}));
            arrFilters.push(search.createFilter({name: 'isquarter', operator: 'is', values: ['F']}));
            arrFilters.push(search.createFilter({name: 'isyear', operator: 'is', values: ['F']}));
            arrFilters.push(search.createFilter({name: 'startdate', operator: 'onorbefore', values: stDate}));
            arrFilters.push(search.createFilter({name: 'enddate', operator: 'onorafter', values: stDate}));

            var arrColumns = [];
            arrColumns.push(search.createColumn({name: 'internalid', summary: 'GROUP'}));

            var arrSearchResults = lib.search('accountingperiod', null, arrFilters, arrColumns);
            if(arrSearchResults.length > 0)
            {
                stPeriodId = arrSearchResults[0].getValue({name: 'internalid', summary: 'GROUP'});
            }
        ...
        /*
        *   Sets values in URL on record
        */
        lib.setParameterFromURL = function (objCurrentRecord)
        {
            var stURL = window.location.search.substring(1);
            var arrSplitURL = stURL.split('&');
            if(arrSplitURL)
            {
                for(var i = 0; i < arrSplitURL.length; i++)
                {
                    var arrPairValues = arrSplitURL[i].split('=');
                    if(arrPairValues)
                    {
                        var stValue = decodeURIComponent(arrPairValues[1]);
                        if(stValue == 'true' || stValue == 'T')
                        {
                            stValue = true;
                        }
                        else if(stValue == 'false' || stValue == 'F')
                        {
                            stValue = false;
                        }
                        console.log('Setting ' + arrPairValues[0] + ' to ' + stValue);
                        if(arrPairValues[0].indexOf('date') > -1 && stValue)
                        {
                            stValue = format.parse({
                                type: format.Type.DATE,
                                value: stValue
                            });
                        }
                        objCurrentRecord.setValue({
                            fieldId: arrPairValues[0],
                            value: stValue,
        ...
        */
        ...
        /**
         * Determine Number of Months between two dates
         */
        lib.getNumberOfMonths = function (stDate1, stDate2)
        {

            stDate1 = format.parse({value: stDate1, type: format.Type.DATE});
            stDate2 = format.parse({value: stDate2, type: format.Type.DATE});
            var stYear1 = stDate1.getFullYear();
            var stYear2 = stDate2.getFullYear();
            var stMonth1 = stDate1.getMonth();
            var stMonth2 = stDate2.getMonth();
            var intNumberOfMonths = (stYear2 - stYear1) * 12 + (stMonth2 - stMonth1) + 1;
