 ...
 * @NApiVersion 2.1
 */
define(['N/https', '../Libs/customscript_amp_lib_keys'], (https, config) => {
  const exports = {};
  /**
   * CREATE PRODUCT
   ...
   *
   */
  exports.deleteProduct = (stItemId) => {
    try {
   ...
   *
   */
  exports.cancelWarrantyContract = (contractId, commit) => {
    try {
      const response = https.post({
        url: `${config.domain}/stores/${config.storeId}/contracts/${contractId}/refund?commit=${commit}`,
        headers: {
          'Content-Type': 'application/json',
          'X-Extend-Access-Token': config.key,
        },
